...
bool CLexer::IsAssignment(std::string string)
{
	return FindStringInVector(m_assignment, string);
}
...
bool CLexer::IsInteger(std::string string)
{
	return FindStringInVector(m_numbers, string);
}
...
TokenType CLexer::GetTokenType(Token token)
{
	...
	{
		str = token.value[i];
		if (!(IsLetter(str) || IsInteger(str) || str == "_" || str == "."))
			break;
		str = token.value[0];
...
void CLexer::Run()
{
	...
	while (getline(m_input, line))
	{
		...
		for (i = 0; i < line.size(); ++i)
		{
			...
			if (IsLetter(ch) || IsInteger(ch) || ch == "." || ch == "_")
			{
				if (identifier)
				{
					token.value += line[i];
